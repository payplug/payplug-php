...
class HttpExceptionTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $exception = new \Payplug\Exception\HttpException('message_content', 'http_response_content', 808);
    ...
    {
        $exception = new \Payplug\Exception\HttpException('message_content', '{"error": "an_error"}', 808);
        $this->assertEquals(array('error' => 'an_error'), $exception->getErrorObject());
    }

    public function testGetErrorObjectWhenErrorIsNotJson()
    {
        $exception = new \Payplug\Exception\HttpException('message_content', '{}not_json{}', 808);
